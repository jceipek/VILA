...
var evaluateDiv = function (a, b, pos, scope) {
  if (M.get(a,'type') === S.T_INT && M.get(b,'type') === S.T_INT) {
    var div = M.get(a, 'value') / M.get(b,'value');
    ...
    return makeError("I don't understand division on "+M.get(a,'type').toString()+" and "+M.get(b,'type').toString()
                    , [a, b], pos);
  }
};

var evaluateEq = function (a, b, pos, scope) {
  if (M.get(a,'type') !== M.get(b,'type')) {
    return makeResult(S.makeBool(false, pos));
  } else if (M.get(a,'type') === S.T_INT || // Don't need to check b because we know the types match
             M.get(a,'type') === S.T_FLOAT ||
             M.get(a,'type') === S.T_BOOL) {
    return makeResult(S.makeBool(M.get(a, 'value') == M.get(b,'value'), pos));
  } else {
    return makeError("I don't understand equivalence checks on "+M.get(a,'type').toString()+" and "+M.get(b,'type').toString()
                    , [a, b], pos);
  }
};

var evaluateNeg = function (node, pos, scope) {
  if (M.get(node,'type') === S.T_INT) {
    return makeResult(S.makeInt(-M.get(node,'value'), pos));
  } else if (M.get(node,'type') === S.T_FLOAT) {
    return makeResult(S.makeFloat(-M.get(node,'value'), pos));
  } else {
    return makeError("I don't understand negation on "+M.get(node,'type').toString()
                    , [node], pos);
  }
};

var evaluateUnaryOperator = function (node, scope) {
  var res = M.get(node,'value');
  if (!S.isTerminal(res)) {
    res = evaluateASTTree(res, scope);
    if (res.status === 'ERROR') {
      return res;
    }
  }
  if (M.get(node,'type') === S.E_NEG) {
    res = evaluateNeg(res.res, M.get(node,'pos'), scope);
  }
...
var evaluateBinaryOperator = function (node, scope) {
  var resA = M.get(node,'valueA');
  var resB = M.get(node,'valueB');
  if (!S.isTerminal(resA)) {
    resA = evaluateASTTree(resA, scope);
  ...
  if (!S.isTerminal(resB)) {
    resB = evaluateASTTree(resB, scope);
  ...
  var res = null;
  var pos = M.get(node, 'pos');
  if (M.get(node,'type') === S.E_ADD) {
    res = evaluateAdd(resA, resB, pos, scope);
  } else if (M.get(node,'type') === S.E_SUB) {
    res = evaluateSub(resA, resB, pos, scope);
  } else if (M.get(node,'type') === S.E_MUL) {
    res = evaluateMul(resA, resB, pos, scope);
  } else if (M.get(node,'type') === S.E_DIV) {
    res = evaluateDiv(resA, resB, pos, scope);
  } else if (M.get(node,'type') === S.E_EQ) {
    res = evaluateEq(resA, resB, pos, scope);
  }
  node._cachedResult = res;
  if (res.status === 'ERROR') {
    return res;
...
var evaluateSymbol = function (node, scope) {
  ...
  var name = M.get(node,'name');
  if (scope.scopeOfSymbol(name) === null) {
    var e = makeError("I can't find "+name.toString()+" in my scope or parent scopes :("
                    , [node], M.get(node,'pos'));
    node._cachedResult = e;
    return e;
  }
  var s = scope.lookupSymbolValue(name);
  s = M.assoc(s, 'pos', M.get(node,'pos'));
...
export default function evaluateASTTree (node, scope) {
  ...
  if (M.get(node,'type') === S.S_ASSIGN) {
    return evaluateAssign(node, scope);
  } else if (S.isUnaryOperator(M.get(node,'type'))) {
    return evaluateUnaryOperator(node, scope);
  } else if (S.isBinaryOperator(M.get(node,'type'))) {
    return evaluateBinaryOperator(node, scope);
  } else if (M.get(node,'type') === S.T_SYM) { // Needs to be above isTerminal because we don't want to evaluate symbols before lookup
    return evaluateSymbol(node, scope);
  } else if (S.isTerminal(M.get(node,'type'))) {
    return evaluateTerminal(node, scope);
  } else {
    return makeError("I don't understand "+M.get(node,'type').toString()
                    , [node], M.get(node,'pos'));
    // S.E_CALL
  }
};